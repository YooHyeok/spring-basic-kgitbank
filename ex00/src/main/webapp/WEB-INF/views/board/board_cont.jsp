...
<body>
	...
	<table border="1">
		...
		<tr>
			<th>제목</th>
			<td>${b.title}</td>
			<%--  ${b.title} EL 즉 표현언어는 b.getTitle()메서드를 호출한 효과와 같다.  --%>
		</tr>
		<tr>
			<th>글 내용</th>
			<td>${b.content}</td>
		...
		<tr>
			...
			<%--  board_edit?bno=번호값이 get방식으로 전달.  --%>
	...
	<script>
		...
		//var bno = 19; //게시판 번호. 	
		var bno = ${b.bno}; //자바스크립트에서 EL 즉, 표현언어를 사용할 수 있다.
		/* 게시판 번호 값을 bno변수에 저장
								var : 변수 정의 예약어. 
		 						bno : 자바스크립트 변수명.		*/
		...
		function getAllList() { //function키워드로 getAllList()함수로 정의
			$.getJSON("/controller/replies/all/" + bno,
						function(data) {//get방식으로 JSON데이터를 가져오기 위한 jQuery비동기식 아작스 함수 => 가져오는것 성공시 data매개변수에 값이 저장됨.
						var str = "";
						$(data).each(function() { //&는 jQuery란 뜻이고, each()는 jQuery함수로서 반복한다는 뜻.
			...
			$.ajax({ //get or post방식을 처리하는 jQuery 비동기식 아작스 함수 => 가장 사용빈도가 높다. , $대신 jQuery를 사용해도 됨
				...
				success : function(result) { //비동기식으로 가져온 것이 성공식 호출되는 콜백함수 => 정상적으로 가져온 데이터는 result매개변수에 저장됨. 
					...
					if (result == 'success') {
						...
						location.reload();//자바스크립트 location객체의 reload()메서드로 새로고침 (단축키:f5)
